...
global.SheetEnvelope = require('./envelope'); // app-layer's toJSON/loadJSON use it
...
test('Store: load re-derives a stale literal cache from raw (never trusts the file)', () => {
  resetStore();
  const legacyV2 = JSON.stringify({
    tool: 'localsheets', version: '2.0', meta: { title: 'stale' }, created: '2020', modified: '2020',
    sheets: { s1: { name: 'Sheet1', cells: {
      A1: { raw: '5', value: 99999, type: 'number' },   // STALE/edited cache: raw says 5, value lies 99999
      A2: { raw: '=A1+1', formula: '=A1+1' }
    }, colWidths: {}, selR: 0, selC: 0 } },
    sheetOrder: ['s1'], activeSheet: 's1', settings: {}
  });
  Store.loadJSON(legacyV2);
  eq(Store.getCell('A1').value, 5);   // re-derived from raw, not the stale 99999
  eq(Store.getCell('A2').value, 6);   // and the dependent formula uses the corrected literal
});

test('Store: malformed cells (null / array / string) load without crashing', () => {
  const mk = cells => JSON.stringify({ tool: 'localsheets', version: '2.0', meta: {}, sheets: { s1: { name: 'S', cells, colWidths: {}, selR: 0, selC: 0 } }, sheetOrder: ['s1'], activeSheet: 's1', settings: {} });
  let ok1 = true, ok2 = true, ok3 = true;
  resetStore(); try { Store.loadJSON(mk(null)); } catch (e) { ok1 = false; }
  resetStore(); try { Store.loadJSON(mk('oops')); } catch (e) { ok2 = false; }
  resetStore(); try { Store.loadJSON(mk([1, 2, 3])); } catch (e) { ok3 = false; }
  eq(ok1, true); eq(ok2, true); eq(ok3, true);
  eq(Object.keys(Store.data.sheets.s1.cells).length, 0);   // garbage coerced to an empty cells map
});
