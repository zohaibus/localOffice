...
const Store = {
  ...
  setCell(coord, rawText, opts = {}) {
    ...
    const validation = existing && existing.validation ? existing.validation : null; // preserve list-validation
    const note = existing && existing.note ? existing.note : null;                   // preserve cell note
  ...
  recalcAll() {
    ...
    for (let pass = 0; pass < 2; pass++) {
      for (const sid of this.data.sheetOrder) {
        ...
        if (!sheet || !sheet.cells || typeof sheet.cells !== 'object') continue;   // guard: malformed cells never crash recalc
  ...
  // Writes the shared localoffice/v1 `sheet` envelope (see src/envelope.js).
  // The in-memory model stays the v2.0 shape; SheetEnvelope translates only here.
  toJSON() {
    if (!this.data._loId) this.data._loId = SheetEnvelope.uuid(); // stable doc id across saves
    this.data.modified = nowISO();
    return JSON.stringify(SheetEnvelope.toEnvelope(this.data, E), null, 2);
  },

  // Reads ALL three formats: the new envelope, legacy v2.0, and legacy v1.x -
  // existing files must never break.
  loadJSON(text) {
    let p;
    try { p = JSON.parse(text); } catch { throw new Error('File is not valid JSON.'); }
    const kind = SheetEnvelope.detectFormat(p);
    if (kind === 'envelope') {
      this.data = SheetEnvelope.fromEnvelope(p, E);
    } else if (kind === 'legacy-v2') {
      this.data = p;                       // already the internal shape
    } else if (kind === 'legacy-v1') {
      this.data = this._migrateV1toV2(p);
    } else if (kind === 'envelope-other') {
      throw new Error(`This is a LocalOffice "${p.type}" file, not a sheet.`);
    } else if (kind === 'legacy-unsupported') {
      throw new Error(`File version ${p.version} not supported (expects v${SUPPORTED_MAJOR}.x).`);
    } else {
      throw new Error('Not a LocalSheets file.');
    }
    if (!this.data.sheets || !this.data.sheetOrder || !this.data.activeSheet) {
      throw new Error('File missing required fields.');
    }
    // Never trust a cached value from a file. Formulas are recomputed by recalcAll;
    // a legacy-v2 file's LITERAL cells kept their stored value/type verbatim, so an
    // edited/stale literal silently poisoned every dependent formula (raw "5" with a
    // bogus value 99999 made =A1+1 read 100000). Re-derive every literal from raw,
    // and harden a malformed `cells` (null/array/string) to an empty object.
    for (const sid of this.data.sheetOrder) {
      const sheet = this.data.sheets[sid];
      if (!sheet) continue;
      if (!sheet.cells || typeof sheet.cells !== 'object' || Array.isArray(sheet.cells)) sheet.cells = {};
      for (const c of Object.values(sheet.cells)) {
        if (!c || typeof c !== 'object' || c.formula) continue;
        const num = E.isStrictNumber(c.raw);
        c.value = num ? Number(c.raw) : c.raw;
        c.type = num ? 'number' : 'text';
      }
    }
    this.undoStack = [];
    this.redoStack = [];
